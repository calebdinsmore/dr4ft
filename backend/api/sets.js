...
setsRouter.post("/upload", (req, res) => {
...
const sanitize = (json) => {
  ...
  json.cards = json.cards.map(
    ({
      //take only values that we need
      name,
      frameEffects,
      number,
      layout,
...
function integrateJson(json) {
  ...
  if (json.code in sets) {
    ...
    if (sets[json.code].type !== CUSTOM_TYPE) {
      throw new Error(
        `Set existing already. Not saving again set with code "${json.code}" to database`
      );
    } else {
      logger.info(
        `Custom set ${json.code} already existing. Overriding with new file...`
      );
    }
  }
  ...
  fs.writeFile(
    path.join(customDataDir, `${json.code}.json`),
