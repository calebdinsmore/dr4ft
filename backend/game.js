...
module.exports = class Game extends Room {
  constructor({
    ...
    Object.assign(this, {
      ...
      firstPackPicks: parseInt(firstPackPicks),
      delta: -1,
      hostID: hostId,
      id: gameID,
      players: [],
    ...
    switch (type) {
      ...
      case "chaos sealed": {
        ...
        chaosOptions.push(
          modernOnly ? "Modern sets only" : "Not modern sets only"
        );
  ...
  static numActiveGames() {
    return Rooms.getAll().filter(({ isActive }) => isActive).length;
  }
  ...
  static getRoomInfo() {
    return Rooms.getAll()
      .filter(
        ({ isPrivate, didGameStart, isActive }) =>
          !isPrivate && !didGameStart && isActive
      )
      .reduce((acc, game) => {
        const usedSeats = game.players.length;
        const totalSeats = game.seats;
        if (usedSeats === totalSeats) return acc;
  ...
  pass(p, pack) {
    if (!pack.length) {
      if (!--this.packCount) this.startRound();
      else this.meta();
      return;
    }
  ...
  createPool() {
    switch (this.type) {
      ...
      default:
        throw new Error(`Type ${this.type} not recognized`);
    }
  }
    ...
    try {
      ...
      if (this.shouldAddBots()) {
        while (this.players.length < this.seats) {
          const burnsPerPack =
            this.type === "cube draft" ? this.cube.burnsPerPack : 0;
          this.players.push(
            new Bot(
              this.picksPerPack,
              burnsPerPack,
      ...
      this.logger.error(
        `Game ${this.id} encountered an error while starting: ${
          err.stack
      ...
      this.players.forEach((player) => {
        if (!player.isBot) {
          player.exit();
          player.err(
            "Whoops! An error occurred while starting the game. Please try again later. If the problem persists, you can open an issue on the Github repository: <a href='https://github.com/dr4fters/dr4ft/issues' target='_blank' rel='noreferrer'>https://github.com/dr4fters/dr4ft/issues</a>"
          );
        }
      });
      Rooms.delete(this.id);
  ...
  toString() {
    ...
    players: ${this.players.length} (${this.players
      .filter((pl) => !pl.isBot)
      .map((pl) => pl.name)
      .join(", ")})
    bots: ${this.bots}
